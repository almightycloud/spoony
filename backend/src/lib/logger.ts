import { createLogger, format as f, Logger as WinstonLogger, transports } from 'winston';
...
if (process.env.NODE_ENV === 'production') {
  format = f.combine(
    ...
    f.splat(),
    f.printf(({ level, message, timestamp }) => `${`[${level}]`.padStart(7)} ${timestamp} ${message}`),
  );
} else {
  format = f.combine(
    f.timestamp({ format: 'YYYY-MM-DD hh:mm:ss:ms A' }),
    f.splat(),
    f.colorize(),
    f.printf(({ level, message, timestamp }) => `${`[${level}]`.padStart(17)} ${timestamp} ${message}`),
  );
}
